...
%First assume $A\leq g^*(B)$.  This means that $A$ is a finer partition of $S$ than $g^*(B)$.  We want to show that $g_!(A)\leq B$, i.e. if $t_1$ and $t_2$  are in the same set in the partition $g_!(A)$, then they must be in the same set in the partition $B$
